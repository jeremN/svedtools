...
<article class="doc">
	<h1>Vite Plugin</h1>
	<p class="lead">
		The Vite plugin instruments Svelte's compiled output at build time, injecting hooks that the
		DevTools extension reads at runtime.
	</p>
	...
	<section>
		<h2>How It Works</h2>
		<p>
			The plugin operates as a Vite transform. After the Svelte compiler produces JavaScript
			output, the plugin parses the AST and injects DevTools hooks around key operations.
		</p>
	...
	<section>
		...
		<p>
			Each component function is wrapped with <code>$.push</code> and <code>$.pop</code> calls
			that track the component tree:
		</p>
		<pre><code>{pushPopExample}</code></pre>

		<h3>State Mutations</h3>
		<p>
			Assignments to <code>$state</code> variables compile to <code>$.set</code> calls. The
			plugin ensures these are visible to the DevTools:
		</p>
	...
	<section>
		...
		<p>
			In addition to AST transforms, the plugin injects a small bridge script into the page.
			This bridge:
		</p>
		<ul>
		...
		<p>
			The bridge is only active when the DevTools panel is open, keeping the overhead
			minimal.
		</p>
	</section>
</article>
