...
<article class="doc">
  ...
  <section>
    <h2>Data Flow</h2>
    <p>Data flows in one direction, from the instrumented application to the DevTools panel:</p>
    <ol>
      <li>
        <strong>Compile time</strong> &mdash; The Vite plugin transforms Svelte's compiled JavaScript output. It injects
        calls to <code>$.push</code>, <code>$.pop</code>,
        <code>$.set</code>, and <code>$.update</code> that emit instrumentation events.
      </li>
      <li>
        <strong>Page bridge</strong> &mdash; A small script injected into the page listens for these events. It
        serializes component data (props, state, file locations) using the shared wire protocol and posts it via
      ...
      <li>
        ...
        instances. It routes incoming messages to the correct panel based on the tab ID.
      </li>
      <li>
  ...
  <section>
    ...
    <p>
      Unlike runtime-based DevTools that monkey-patch framework internals, this approach modifies the compiled output at
    ...
    <p>
      ...
      symbols). The wire protocol handles this by:
    </p>
    <ul>
  ...
  apps/
    docs/           # This documentation site (SvelteKit + static adapter)</code
      ></pre>
  </section>
</article>
