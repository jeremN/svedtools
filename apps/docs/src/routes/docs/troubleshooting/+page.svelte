...
<article class="doc">
  ...
  <section>
    ...
    <ol>
      <li>
        The page must be served by a <strong>dev server</strong> with the plugin registered after <code>svelte()</code>.
        The Vite plugin only ever activates on <code>apply: 'serve'</code> &mdash; production and static builds never inject
        the runtime bridge, so the panel has nothing to talk to.
      </li>
      <li>
  ...
  <section>
    ...
    <p>
      The detected Svelte major version falls outside the range this bridge is verified against &mdash; see
      <a href={resolve('/docs/vite-plugin')}>Vite Plugin</a> for the current range. DevTools features may misbehave on an
      untested major; the component tree and most panel features typically still work.
    </p>
  </section>
  ...
  <section>
    ...
    <p>
      ...
      doesn't emit. On those versions the component tree, state inspection, and tracing all still work &mdash; only the source-code
  ...
  <section>
    ...
    <ul>
      <li><code>$derived</code> and <code>$props</code> rows are never editable.</li>
      ...
      <li>
        ...
        the value's type &mdash; retype it as valid JSON of the intended type.
      </li>
    </ul>
